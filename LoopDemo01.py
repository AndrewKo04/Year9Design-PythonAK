#Loops are structures use to repeat sections of code.
#They are useful if you have ot do the samr thing more than once
#or you can establish a pattern
      

#Example 1:
print ("0")
print ("1")
print ("2")
print ("3")
print ("4")

#This is counted loops. I want you to think about
#count, check, change
# i = 0,0 < 5, TRUE - RUN LOOP
# i = 1,1 < 5, TRUE - RUN LOOP
# i = 2,2 < 5, TRUE - RUN LOOP
# i = 3,3 < 5, TRUE - RUN LOOP
# i = 4,4 < 5, TRUE - RUN LOOP
# i = 5,5 < 5, FALSE - EXIT AND MOVE O
for i in range (4,12,2):
	#ANYTHING TABBED IS CONSIDERED THE LOOP BLOCK 
	print (i*2)

print ("***********")

for i in range(2,6,1):
		print(i*2)

print ("*********BACKWARDS**********")
#If you change your increment to go in reverse 
#the check is always i > check in this case -1
for i in range (10,-1,-1)
	print(i)

print("************Printing String Characters******")
str= "Monkey"
#We cna use the loop to go through each index 
#in a string to print out every letter
for i in range (0,6, 1):
	print(str(i))

print ("MOVING ON")

print ("********PRINT STRING IN REVERSE*********")
for i in range(len(str)- 1, -1, -1):
	print(str[i])

print("********* PRINT EVERY SECOND LETTER IN STR STAR AT INDEX 0*******")
for i

	Credit

